  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect(
      ...
      child: dataProvider.popularMoviesList.isEmpty
                      ? const Center()
                      :CachedNetworkImage(
      imageUrl: movie.posterURL(),// Utilisation de CachedNetworkImage pour le chargement d'images
      ...
      errorWidget: (context, url, error) => const Icon(Icons.error),// Affiche une icône d'erreur si le chargement échoue
       ...
       placeholder: (context, url) =>  Center(child: CircularProgressIndicator(color: kPrimaryColor,)),// Affiche un indicateur de chargement pendant le chargement de l'image
      // errorWidget: (context, url, error) => const Icon(Icons.error),// Affiche une icône d'erreur si le chargement échoue
      // fit: BoxFit.cover,// Ajuste l'image pour couvrir tout l'espace disponible
    )
    );
